...
namespace ToolLibrary
{
	...
	public class BSTree : IBSTree
	{
		...
		{
			return Search(firstName, lastName, root);
		}

		public bool SearchAndLogin(string firstName, string lastName, string PIN)
		{
			return SearchAndLogin(firstName, lastName, PIN, root);
		}
		...
		private bool SearchAndLogin(string firstName, string lastName, string PIN, BTreeNode r)
		{
			if (r != null)
			{
				if (firstName.CompareTo(r.Item.FirstName) == 0 && 
					lastName.CompareTo(r.Item.LastName) == 0 && 
					PIN.CompareTo(r.Item.PIN) == 0)
                {
					return true;
                } else if (lastName.CompareTo(r.Item.LastName) == 0) 
                {
					return SearchAndLogin(firstName, lastName, PIN, r.LChild);
				} else
                {
					return SearchAndLogin(firstName, lastName, PIN, r.RChild);
                }
			}
			else
				return false;
		...
		private string Search(string firstName, string lastName, BTreeNode r) //Searching for phone number from given first/last name
		{
			if (r != null)
			{
				if (firstName.CompareTo(r.Item.FirstName) == 0 &&
					lastName.CompareTo(r.Item.LastName) == 0)
				{
					...
					return r.Item.ContactNumber;
				}
				else if (lastName.CompareTo(r.Item.LastName) == 0)
				{
					return Search(firstName, lastName, r.LChild);
				}
				else
				{
					return Search(firstName, lastName, r.RChild);
				}
			}
			else
				return "-1";
